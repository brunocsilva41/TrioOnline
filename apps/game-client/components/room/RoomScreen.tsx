...
export default function RoomScreen() {
  ...
  const handleToggleReady = () => !isProcessing && colyseusService.sendReady();
  const handleStartGame = () => !isProcessing && colyseusService.sendStartGame();
  const handleLeave = () => !isProcessing && colyseusService.leaveRoom();
    ...
    >
      ...
      >
        <div className="flex items-center justify-center gap-3 mb-4 w-full">
          ...
          <h2 className="text-[10px] sm:text-xs font-black font-display tracking-[0.5em] text-emerald-400/80 uppercase whitespace-nowrap">
        ...
        {roomCode && (
          <div className="flex justify-center">
            ...
            >
              ...
              <span className="text-3xl sm:text-4xl font-black font-display tracking-[0.3em] text-amber-400 drop-shadow-[0_0_10px_rgba(251,191,36,0.3)]">{roomCode}</span>
      ...
      <div className="sm:relative fixed bottom-0 left-0 right-0 sm:bottom-auto sm:left-auto sm:right-auto z-[60] bg-[#020617]/80 backdrop-blur-lg sm:bg-transparent border-t border-white/10 sm:border-t-0 p-4 sm:p-0">
        ...
        >
          <motion.button
            onClick={handleLeave}
            whileTap={{ scale: 0.95 }}
            className="w-full sm:w-auto px-10 sm:px-12 py-5 sm:py-6 bg-white/5 hover:bg-rose-500/15 border border-white/10 hover:border-rose-500/40
              rounded-[1.5rem] text-xs font-black font-display tracking-widest text-white/60 hover:text-rose-400 transition-all uppercase"
          >
            SAIR
          </motion.button>
          ...
          {isHost ? (
            <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="w-full sm:w-auto px-8 sm:px-10 py-5 sm:py-6 rounded-[1.5rem] bg-red-900/30 border border-red-500/30 text-xs font-black font-display tracking-widest uppercase text-red-400/80 hover:bg-red-900/50 hover:text-red-300 hover:border-red-400/50 transition-all shadow-lg"
              >
                FECHAR SALA
              </motion.button>
              <motion.button
                ...
                className={`w-full sm:w-auto px-12 sm:px-20 py-5 sm:py-6 rounded-[1.5rem] font-black font-display text-sm sm:text-base tracking-[0.2em] uppercase transition-all ${
                  allReady
                    ? 'bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-black shadow-[0_15px_40px_rgba(16,185,129,0.3)] hover:shadow-[0_20px_60px_rgba(16,185,129,0.5)]'
            ...
            <motion.button
              ...
              className={`w-full sm:w-auto px-12 sm:px-20 py-5 sm:py-6 rounded-[1.5rem] font-black font-display text-sm sm:text-base tracking-[0.2em] uppercase transition-all ${
                players[mySessionId]?.isReady
                  ? 'bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-black shadow-[0_15px_40px_rgba(251,191,36,0.3)]'
                  : 'bg-white/5 hover:bg-emerald-500/15 border border-emerald-500/40 text-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.1)]'
              }`}
            >
      ...
      {!isHost && players[mySessionId]?.isReady && (
        <motion.p
          ...
          className="z-10 mt-6 text-[10px] text-amber-400/60 font-black font-display tracking-[0.3em] uppercase drop-shadow-md"
        >
